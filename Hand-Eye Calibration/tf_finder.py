...
def find_tf(data1, data2):
    def tf_error(tf_x):
        error=np.sum(
            np.sum(np.square((get_tf(tf_x) * augment_data(data1).transpose() - augment_data(data2).transpose()))))
        return error

    x0 = np.array([0, 0, 0, 0, 0, 0])
    x1 = np.c_[(np.random.rand(10, 3) * 2 - 1) * math.pi, np.random.rand(10, 3) * 2 - 1]
    res = minimize(tf_error, x0, method='nelder-mead', options={'maxiter': 10000, 'xtol': 1e-10, 'disp': False})
    current_fun = res.fun
    current_x = res.x
    for x in x1:
        res = minimize(tf_error, x, method='nelder-mead', options={'maxiter': 10000, 'xtol': 1e-10, 'disp': False})
